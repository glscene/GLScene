...
procedure SetBB(var C: THmgBoundingBox; const V: TgxVector);
procedure SetAABB(var Bb: TAABB; const V: TgxVector); inline;

procedure BBTransform(var C: THmgBoundingBox; const M: TgxMatrix);
procedure AABBTransform(var Bb: TAABB; const M: TgxMatrix);
...
procedure AABBFromSweep(var SweepAABB: TAABB; const Start, Dest: TgxVector; const Radius: Single);
...
function AABBToBB(const AnAABB: TAABB; const M: TgxMatrix): THmgBoundingBox; overload;
//  Adds delta to min and max of the AABB.
procedure OffsetAABB(var Aabb: TAABB; const Delta: TAffineVector); overload;
procedure OffsetAABB(var Aabb: TAABB; const Delta: TgxVector); overload;
//  Adds delta to min and max of the BB.
procedure OffsetBB(var Bb: THmgBoundingBox; const Delta: TAffineVector); overload;
procedure OffsetBB(var Bb: THmgBoundingBox; const Delta: TgxVector); overload;
// The same as above but uses AddPoint() instead of AddVector().
procedure OffsetBBPoint(var Bb: THmgBoundingBox; const Delta: TgxVector); overload;
...
function IntersectAABBs(const Aabb1, Aabb2: TAABB; const M1To2, M2To1: TgxMatrix)
  : Boolean; overload;
...
function PointInAABB(const P: TgxVector; const Aabb: TAABB): Boolean; overload;
...
function BSphereToAABB(const Center: TgxVector; Radius: Single): TAABB; overload;
...
function AABBToClipRect(const Aabb: TAABB; const ModelViewProjection: TgxMatrix;
  ViewportSizeX, ViewportSizeY: Integer): TClipRect;

// Finds the intersection between a ray and an axis aligned bounding box.
function RayCastAABBIntersect(const RayOrigin, RayDirection: TgxVector;
  const Aabb: TAABB; out TNear, TFar: Single): Boolean; overload;
function RayCastAABBIntersect(const RayOrigin, RayDirection: TgxVector;
  const Aabb: TAABB; IntersectPoint: PgxVector = nil): Boolean; overload;
...
procedure SetAABB(var Bb: TAABB; const V: TgxVector);
begin
  Bb.Max.X := Abs(V.X);
  Bb.Max.Y := Abs(V.Y);
  Bb.Max.Z := Abs(V.Z);
...
procedure AABBFromSweep(var SweepAABB: TAABB; const Start, Dest: TgxVector;
  const Radius: Single);
begin
...
function AABBToBB(const AnAABB: TAABB; const M: TgxMatrix): THmgBoundingBox;
begin
...
procedure OffsetAABB(var Aabb: TAABB; const Delta: TgxVector);
begin
  AddVector(Aabb.Min, Delta);
  AddVector(Aabb.Max, Delta);
end;
...
procedure OffsetBB(var Bb: THmgBoundingBox; const Delta: TgxVector);
var
  I: Integer;
begin
  for I := 0 to 7 do
...
procedure OffsetBBPoint(var Bb: THmgBoundingBox; const Delta: TgxVector);
var
  I: Integer;
begin
  for I := 0 to 7 do
...
function IntersectAABBs(const Aabb1, Aabb2: TAABB;
  const M1To2, M2To1: TgxMatrix): Boolean;
const
...
function PointInAABB(const P: TgxVector; const Aabb: TAABB): Boolean;
begin
  Result := (P.X <= Aabb.Max.X) and
    (P.X >= Aabb.Min.X) and (P.Y <= Aabb.Max.Y) and
    (P.Y >= Aabb.Min.Y) and (P.Z <= Aabb.Max.Z) and
...
function BSphereToAABB(const Center: TgxVector; Radius: Single): TAABB;
begin
