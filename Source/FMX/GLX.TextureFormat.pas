...
// Give a GLXcene texture format from OpenGL texture format.
...
begin
  case glTarget of
    GL_TEXTURE_1D: Result := True; // GL_VERSION_1_1 or GL_EXT_texture_object;
    GL_TEXTURE_2D: Result := True; //GL_VERSION_1_1 or GL_EXT_texture_object;
    GL_TEXTURE_3D: Result := True; //GL_EXT_texture3D;
    GL_TEXTURE_RECTANGLE: Result := True; //GL_ARB_texture_rectangle;
    GL_TEXTURE_CUBE_MAP,
      ...
      GL_TEXTURE_CUBE_MAP_NEGATIVE_Z: Result := True; //GL_ARB_texture_cube_map;
    GL_TEXTURE_1D_ARRAY: Result := True; //GL_EXT_texture_array;
    GL_TEXTURE_2D_ARRAY: Result := True; //GL_EXT_texture_array;
    GL_TEXTURE_CUBE_MAP_ARRAY: Result := True; //GL_ARB_texture_cube_map_array;
    GL_TEXTURE_BUFFER: Result := True; //GL_ARB_texture_buffer_object;
    GL_TEXTURE_2D_MULTISAMPLE,
      GL_TEXTURE_2D_MULTISAMPLE_ARRAY: Result := True; //GL_ARB_texture_multisample;
  else
    begin
      Result := false;
...
begin
  ...
  begin
    Result := True; // GL_EXT_texture_compression_s3tc;
    EXIT;
  end;
  ...
  begin
    Result := True; //GL_ARB_texture_float or GL_ATI_texture_float;
    EXIT;
  end;

  if intFormat = tfDEPTH24_STENCIL8 then
  begin
    Result := True; //GL_EXT_packed_depth_stencil;
    EXIT;
  end;

  if ((intFormat = tfDEPTH_COMPONENT32F) or (intFormat = tfDEPTH32F_STENCIL8)) then
  begin
    Result := True; //GL_NV_depth_buffer_float;
    EXIT;
  end;
  ...
  begin
    Result := True; //GL_EXT_texture_sRGB;
    EXIT;
  end;

  if intFormat = tfRGB9_E5 then
  begin
    Result := True; //GL_EXT_texture_shared_exponent;
    EXIT;
  end;

  if intFormat = tfR11F_G11F_B10F then
  begin
    Result := True; //GL_EXT_packed_float;
    EXIT;
  end;
  ...
  begin
    Result := True; //GL_EXT_texture_compression_latc;
    EXIT;
  end;

  if ((intFormat >= tfRGBA32UI) and (intFormat <= tfLUMINANCE_ALPHA8I)) then
  begin
    Result := True; //GL_EXT_texture_integer;
    EXIT;
  end;
