...
procedure TGLMD5VectorFile.LoadFromStream(aStream: TStream);
  ...
  begin
    ...
    pos.X := GLS.Utils.StrToFloatDef(FTempString[2]);
    pos.Y := GLS.Utils.StrToFloatDef(FTempString[4]);
    pos.Z := GLS.Utils.StrToFloatDef(FTempString[3]);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            mesh.TexCoords[vnum] :=
              AffineVectorMake(GLS.Utils.StrToFloatDef(FTempString[2]),
              1 - GLS.Utils.StrToFloatDef(FTempString[3]), 0);
        ...
        begin
          ...
          begin
            ...
            VertexWeighted[wnum] :=
              AffineVectorMake(GLS.Utils.StrToFloatDef(FTempString[4]),
              GLS.Utils.StrToFloatDef(FTempString[6]),
  ...
  begin
    ...
    repeat
      ...
      if FTempString.Count >= 6 then
      begin
        pos := AffineVectorMake(GLS.Utils.StrToFloatDef(FTempString[0]),
          GLS.Utils.StrToFloatDef(FTempString[1]),
          GLS.Utils.StrToFloatDef(FTempString[2]));
        quat := QuaternionMakeFromImag(GLS.Utils.StrToFloatDef(FTempString[3]),
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          pos.X := GLS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 2 > 0 then
        begin
          pos.Y := GLS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 4 > 0 then
        begin
          pos.Z := GLS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;

        if FJointFlags[i] and 8 > 0 then
        begin
          quat.ImagPart.X := GLS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 16 > 0 then
        begin
          quat.ImagPart.Y := GLS.Utils.StrToFloatDef(FTempString[j]);
          Inc(j);
        end;
        if FJointFlags[i] and 32 > 0 then
          quat.ImagPart.Z := GLS.Utils.StrToFloatDef(FTempString[j]);
      end;
