...
type
  ...
  *)
  ...
  public
    ...
    constructor Create(aOwner: TGLPhysicsManager); // override; //abstract;
  ...
  published
    ...
    property Manager: TGLPhysicsManager read FManager write SetManager;
  end;
  ...
  (* Physics manager can only deal with objects from one scene
    More than one physics manager can be assigned to a scene *)
  TGLPhysicsManager = class(TComponent)
    // StateSize:Integer;
  protected
...
procedure TGLPhysicsManager.DefineProperties(Filer: TFiler);
begin
...
procedure TGLPhysicsManager.Loaded;
begin
  inherited Loaded;
...
function TGLPhysicsManager.FindObjectByName(Name: String): TGLBaseSceneObject;
var
  i: Integer;
begin
  Result := nil;
...
function TGLPhysicsManager.FindForceFieldEmitterByName(Name: String)
  : TGLBaseSceneObject;
var
  i: Integer;
begin
...
procedure TGLPhysicsManager.WriteForces(stream: TStream);
var
  writer: TWriter;
begin
...
procedure TGLPhysicsManager.ReadForces(stream: TStream);
var
  reader: TReader;
begin
...
procedure TGLPhysicsManager.SetForces(const val: TGLForces);
begin
  Forces.Assign(val);
end;

procedure TGLPhysicsManager.SetInertias(const val: TList);
begin
  fInertias.Assign(val);
end;

procedure TGLPhysicsManager.SetForceFieldEmitters(const val: TList);
begin
  fForceFieldEmitters.Assign(val);
end;

procedure TGLPhysicsManager.SetScene(const val: TGLScene);
begin
  // fScene:=val;
  if fScene <> val then
  begin
...
function TGLPhysicsManager.GetForces: TGLForces;
begin
...
constructor TDESolver.Create(aOwner: TGLPhysicsManager);
begin
  Self.Owner := aOwner;
end;
...
constructor TGLPhysicsManager.Create(aOwner: TComponent);
begin
...
destructor TGLPhysicsManager.Destroy;
begin
  // fScene:=nil;
...
procedure TGLPhysicsManager.Assign(Source: TPersistent);
begin
  inherited Assign(Source);
end;

procedure TGLPhysicsManager.SetDESolver(SolverType: TDESolverType);
var
...
procedure TGLPhysicsManager.RegisterInertia(aInertia: TGLBaseInertia);
begin
...
procedure TGLPhysicsManager.DeRegisterInertia(aInertia: TGLBaseInertia);
begin
  if Assigned(aInertia) then
  begin
...
procedure TGLPhysicsManager.DeRegisterAllInertias;
var
  i: Integer;
begin
  // Fast deregistration
...
procedure TGLPhysicsManager.DeRegisterForceFieldEmitter
  (aForceField: TGLBaseForceFieldEmitter);
begin
  if Assigned(aForceField) then
  begin
...
procedure TGLPhysicsManager.DeRegisterAllForceFieldEmitters;
var
  i: Integer;
begin
  // Fast deregistration
...
function TGLPhysicsManager.CalculateKE(): Real;
var
  Total: Real;
  i: Integer;
begin
...
function TGLPhysicsManager.CalculatePE(): Real;
var
  Total: Real;
  i: Integer;
begin
...
procedure TGLPhysicsManager.CalculateNextState(DeltaTime: Real);
begin
  if Assigned(DESolver) then
...
procedure TGLBaseInertia.SetManager(const val: TGLPhysicsManager);
begin
...
begin
  ...
  begin
///?    mng := FindManager(TGLPhysicsManager, FManagerName);
    if Assigned(mng) then
      Manager := TGLPhysicsManager(mng);
    FManagerName := '';
  end;
end;
...
procedure TGLBaseForceFieldEmitter.SetManager(const val: TGLPhysicsManager);
begin
  if val <> FManager then
  begin
    if Assigned(FManager) then
...
// RegisterClasses([TGLPhysicsManager, TGLBaseInertia, TGLBaseForceFieldEmitter]);
